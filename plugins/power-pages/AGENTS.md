...
- **Shell-agnostic docs** — SKILL.md, agent, and reference files must not embed shell-specific syntax. Use ` ```bash ` fences (or plain ` ``` `) only for cross-platform commands like `pac`, `az`, `dotnet`, and `node`. Do not use PowerShell cmdlets (`Get-ChildItem`, `Test-Path`, `New-Item`, `Get-Content`, `Remove-Item`, `ConvertFrom-Json`, `Invoke-RestMethod`, etc.) or PowerShell-only syntax like `$var = command` captures and `$variable` references — prefer `<placeholder>` angle-bracket style (e.g., `<envUrl>`). For filesystem and JSON operations the agent already has first-class tools (`Glob`, `Read`, `Write`, `Edit`) — describe the intent in prose rather than prescribing a shell command.
